...
if( isset($_POST['name']) && empty($_POST['spam-check']) ) {
	...
	$input_design = strip_tags($_POST['design']);
	$input_redesign = strip_tags($_POST['redesign']);
	$input_seo = strip_tags($_POST['seo']);
	$input_marketing = strip_tags($_POST['marketing']);
	$input_other = strip_tags($_POST['other']);
	$input_domain = strip_tags($_POST['domain']);
	$input_graphic = strip_tags($_POST['graphic']);
	$input_wordpress = strip_tags($_POST['wordpress']);
	$input_development = strip_tags($_POST['development']);
	$input_apps = strip_tags($_POST['apps']);
	$input_findus = strip_tags($_POST['findus']);
	
	
	// We'll check and see if any of the required fields are empty
	if( strlen($input_name) < 2 ) $error['name'] = 'Please enter valid name.';
	...
	$message .= "\n---\nThis email was sent by GD contact form.";
