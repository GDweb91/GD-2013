...
/* Get the field name of the field to use as "Subject" into the email sent to the site owner */
function getCustomSubjectFieldNames()
{
    global $fields;
...
function sendEmailWithInformations()
{
    ...
    $customEmailFromName = "";
    if (count($customEmailFromNameFieldName) > 0) {
        foreach($customEmailFromNameFieldName as $tmpName) {
            $customEmailFromName .= ($customEmailFromName == "" ? "" : " ") . $fdata[$tmpName];
        }
    }

    if ($customEmailFromFieldName != null) {
        $mail->From = $fdata[$customEmailFromFieldName];
        $mail->FromName = $customEmailFromName != "" ? $customEmailFromName : '';
    } else {
        $mail->From = $configs['from_address'];
        $mail->FromName = $customEmailFromName != "" ? $customEmailFromName : $configs['from_name'];
    }
    ...
    // Set subject
    $customSubjectFieldNames = getCustomSubjectFieldNames();
    if (count($customSubjectFieldNames) > 0) {
        $subject = $lang->emails_info->custom_before_subject;
        foreach($customSubjectFieldNames as $subjectFieldName) {
            $subject .= $fdata[$subjectFieldName] . " ";
        }
...
function sendConfirmationEmailToUser()
{
    ...
    // If attachments are filled in the configs, add them to the email
    foreach ($configs['attachments'] as $filePath) {
        // Add the file to the mail
        $mail->addAttachment(realpath($filePath), basename($filePath));
    }
