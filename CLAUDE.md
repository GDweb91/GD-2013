...
- `seo-services-everett-ma.html` → `organic-SEO-everett-ma.html` (2026-07-15: confirmed correct direction — the live server had this backwards for an unknown period, redirecting `organic-SEO-everett-ma.html` → `seo-services-everett-ma.html` instead, silently killing the page's own search visibility. `organic-SEO-everett-ma.html` is the one every page's nav links to and the one GSC shows with real impressions (7,333 vs. 302 over trailing 12mo) — confirmed via a live `.htaccess` pulled from the server and diffed against this repo. Fixed by re-uploading this repo's `.htaccess`. **Verified live 2026-07-15**: an initial FTP upload of `.htaccess` silently failed to take effect — likely a hidden-dotfile/transfer-mode issue in the FTP client, not a cPanel-Redirects-panel conflict — and pasting the file's contents directly into cPanel File Manager's editor is what actually applied it. Confirmed via `curl -I` against the live URLs: `seo-services-everett-ma.html` → 301 → `organic-SEO-everett-ma.html` (200), and `freelance-web-developer-boston-ma.html` / `freelance-wordpress-website-designer-ma.html` → 301 → `wordpress-developer-boston-ma.html`. **Lesson: after any `.htaccess` edit, always verify with `curl -I` against the live URL — an FTP "upload succeeded" or a visual file comparison is not sufficient confirmation that the change is actually active.**)
...
- `small-business-web-developer-web-designer-boston-ma.html` → `web-design-company-boston-ma.html` (2026-07-16: consolidated — 0 clicks / 294 impressions over trailing 12mo, near-identical title/H1 targeting to the flagship page ("web design + small business + Boston + WordPress"), and shared several cannibalized queries — e.g. "affordable web design boston" split across this page, the flagship, and `wordpress-developer-boston-ma.html`. Internal links from `web-design-company-boston-ma.html` and `web-site-designs-Boston-MA.html`, plus duplicate `sitemap.html`/`sitemap.xml` entries, repointed or removed. **Verified live 2026-07-17**: confirmed via `curl -I` — 301 → `web-design-company-boston-ma.html`.)
- `social-media-advertising.html` → `local-marketing-company-everett-malden-medford-revere-saugus-ma.html` (2026-07-16: page retired at the owner's request, moved to `archive/unsed-pages/`. The "Social Media Integration" / "Redes Sociales" nav item on all 55 pages now points at the local-marketing page instead of being removed, since social media promotion is already covered there. `scripts/fix_navs.py` updated and re-run to apply the new nav target site-wide; `sitemap.html`/`sitemap.xml` entries removed. **Verified live 2026-07-17**: confirmed via `curl -I` — 301 → `local-marketing-company-everett-malden-medford-revere-saugus-ma.html`.)

**Second deployment gotcha found 2026-07-17 (distinct from the FTP silent-failure issue above):** after uploading a correct `.htaccess` via cPanel File Manager for the two redirects above, both URLs kept returning `200` instead of `301` for a while. Root cause: this host (Bluehost) runs an nginx edge/reverse-proxy cache in front of Apache — visible via `x-nginx-cache` / `x-endurance-cache-level` response headers — that caches `200` responses for up to 2 hours (`cache-control: max-age=7200`) and serves them without the request ever reaching Apache. A page that was cached as `200` *before* its redirect rule went live keeps serving that stale copy until the cache entry expires or is purged, no matter how correct the `.htaccess` is. **The `server:` response header tells the two failure modes apart**: `server: nginx` + `x-proxy-cache: HIT` means the edge cache answered (a caching problem, not a config problem); `server: Apache` on a `200` means Apache itself isn't applying the rule (an actual `.htaccess` content/upload problem — re-check the file). Bluehost's cPanel dashboard only exposes a cache-purge button for the WordPress (`/gd-blog/`) side by default; that did not clear the two static-page cache entries. The fix was locating a separate, URL-specific cache-clear option elsewhere in the Bluehost dashboard (outside the WP purge UI). **Lesson: when a live redirect check shows `200` where `301` is expected, check the `server:` header first — it tells you whether to keep digging in `.htaccess` or go clear a cache — and always confirm any fix with a fresh `curl -I` against the live URL.**
- `contact-gd-website-design-estimates.html` → `/gd-blog/contact-us/` (2026-07-20: this line previously claimed a `.htaccess` 301 already existed — it didn't. Found via a GSC page-list cross-reference against local files and existing `.htaccess` sources: the file was archived out of this repo on 2026-07-13 [`archive/unsed-pages/`] but the live copy on the server was never removed, so the old URL kept serving `200` with no redirect. Now has a real rule.)
- `contacts.html` → `/gd-blog/contact-us/` (2026-07-20: same false-claim correction as above — no local file at all, no prior `.htaccess` rule, live `200`. Its own canonical pointed at `contact-gd-website-design-estimates.html`, a two-hop chain instead of a direct redirect.)
- `contact-thanks.html` → `/gd-blog/contact-us/` (2026-07-20: same false-claim correction — no local file, live `200`. Notably its canonical said `/gd-blog/contact-us/` but a later, conflicting `<meta name="robots" content="index, follow, noydir">` tag left it actually indexable despite that intent — a real 301 was needed rather than relying on the page's own contradictory meta tags.)
